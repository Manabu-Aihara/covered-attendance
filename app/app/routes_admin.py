...
from app.models import User, Shinsei, StaffLoggin, Todokede, RecordPaidHoliday, CountAttendance, TimeAttendance, SystemInfo, CounterForTable, D_JOB_HISTORY, KinmuTaisei, Jobtype
...
def edit_user_history(STAFFID, ProcFlag):
    ...
    if ProcFlag == 1:
        ...
        for num in range(1, len(JOBTYPE)):
            AddHISTORY = D_JOB_HISTORY(STAFFID, JOBTYPE[num], CONTYPE[num], blankCheck(PTIME[num]), blankCheck(SDAY[num]), blankCheck(EDAY[num]))
    ...
    History = (
            db.session.query(D_JOB_HISTORY, Jobtype.NAME.label('JOBNAME'), KinmuTaisei.NAME.label('CONTRACTNAME'))
    ...
    ListJob = GetPullDownList(Jobtype, Jobtype.JOBTYPE_CODE, Jobtype.NAME, Jobtype.JOBTYPE_CODE)
    ListCon = GetPullDownList(KinmuTaisei, KinmuTaisei.CONTRACT_CODE , KinmuTaisei.NAME, KinmuTaisei.CONTRACT_CODE)
    StaffInfo = StaffLoggin.query.filter_by(STAFFID=STAFFID).first()
  
        
    return render_template('admin/user_history_edit.html', History=History, StaffInfo=StaffInfo, stf_login=stf_login, ListJob=ListJob,
                           ListCon=ListCon)
