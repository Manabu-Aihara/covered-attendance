...
from app.models import User, Shinsei, StaffLoggin, Todokede, RecordPaidHoliday, D_HOLIDAY_HISTORY, CountAttendance, TimeAttendance, D_JOB_HISTORY, M_TIMECARD_TEMPLATE
...
def indextime(STAFFID, intFlg):
    ...
    tbl_clm = ["日付", "曜日", "oncall", "oncall対応件数", "angel対応件数",
               "開始時間", "終了時間", "走行距離", "届出<br>（午前）", "残業申請", "備考", "届出（午後）"]
    wk = ["日", "土", "月", "火", "水", "木", "金"]
    ...
    db.session.query(Shinsei)
        .filter(and_(Shinsei.STAFFID == STAFFID, 
    ...
    template = (
        ...
        .filter(and_(D_JOB_HISTORY.STAFFID == STAFFID, 
                        D_JOB_HISTORY.START_DAY <= FromDay, D_JOB_HISTORY.END_DAY >= ToDay,
                        D_JOB_HISTORY.JOBTYPE_CODE == M_TIMECARD_TEMPLATE.JOBTYPE_CODE, 
                        D_JOB_HISTORY.CONTRACT_CODE == M_TIMECARD_TEMPLATE.CONTRACT_CODE)).group_by(M_TIMECARD_TEMPLATE.TEMPLATE_NO)
    )
    #月の途中の契約変更1回までは対応
    ...
    shinseis = (
        db.session.query(Shinsei.STAFFID,Shinsei.STARTTIME,Shinsei.ENDTIME,Shinsei.WORKDAY,Shinsei.HOLIDAY,
                            ...
                            Shinsei.ONCALL_COUNT, Shinsei.ENGEL_COUNT, Shinsei.ALCOHOL, Shinsei.REMARK,
                            User.FNAME, User.LNAME,
                            D_JOB_HISTORY.JOBTYPE_CODE, D_JOB_HISTORY.CONTRACT_CODE, 
        ...
        .filter(and_(Shinsei.STAFFID == STAFFID, 
                     Shinsei.STAFFID == User.STAFFID, 
                        ...
                        D_JOB_HISTORY.JOBTYPE_CODE == M_TIMECARD_TEMPLATE.JOBTYPE_CODE, 
                        D_JOB_HISTORY.CONTRACT_CODE == M_TIMECARD_TEMPLATE.CONTRACT_CODE)).order_by(Shinsei.WORKDAY)
        )
    ...
    if form.validate_on_submit():
        ...
        # 削除処理
        delAttendance = db.session.query(Shinsei).filter(and_(Shinsei.STAFFID == STAFFID, Shinsei.WORKDAY.between(FromDay,ToDay)))
        if delAttendance:
        ...
        for c in cal:
            ...
            ##### 勤怠条件分け ##### 
            InsertFlg = 0                        
            atd = AttendanceAnalysys(c, data0, data1, data2, data3, data_4, data5, data6, data7, data8, data9, data10, data11, data12,STAFFID, InsertFlg)            
            atd.analysys()
    ...
    Parttime = db.session.query(D_HOLIDAY_HISTORY.STAFFID, Shinsei.WORKDAY,D_HOLIDAY_HISTORY.HOLIDAY_TIME).filter(
                                    and_(Shinsei.STAFFID == STAFFID, 
                                         Shinsei.STAFFID == D_HOLIDAY_HISTORY.STAFFID,
                                         Shinsei.WORKDAY.between(FromDay,ToDay),
                                         Shinsei.STAFFID == D_HOLIDAY_HISTORY.STAFFID,
                                         D_HOLIDAY_HISTORY.START_DAY <= Shinsei.WORKDAY,
                                         D_HOLIDAY_HISTORY.END_DAY >= Shinsei.WORKDAY)).subquery()

    shinseis = (
        db.session.query(Shinsei.STAFFID,Shinsei.STARTTIME,Shinsei.ENDTIME,Shinsei.WORKDAY,Shinsei.HOLIDAY,
                            Shinsei.OVERTIME,Shinsei.NOTIFICATION, Shinsei.NOTIFICATION2, 
                            Shinsei.ONCALL, Shinsei.ENGEL_COUNT, Shinsei.MILEAGE, Shinsei.ALCOHOL, Shinsei.REMARK, 
        ...
        ).outerjoin(Parttime, and_(Parttime.c.STAFFID == Shinsei.STAFFID, Parttime.c.WORKDAY == Shinsei.WORKDAY)).order_by(Shinsei.STAFFID, Shinsei.WORKDAY)
    ...
    for Shin in shinseis:
        ...
        # 参照モード
        dtm = datetime.strptime(Shin.ENDTIME, '%H:%M') - datetime.strptime(Shin.STARTTIME, '%H:%M')
        real_time = dtm
        # 常勤看護師の場合

        settime = CalcTimeClass(dtm, Shin.NOTIFICATION, Shin.NOTIFICATION2, Shin.STARTTIME, Shin.ENDTIME, Shin.OVERTIME,
                Shin.CONTRACT_CODE, AttendanceDada, over_time_0, 
                    real_time, real_time_sum, syukkin_holiday_times_0, Shin.HOLIDAY, Shin.JOBTYPE_CODE, STAFFID, Shin.WORKDAY,Shin.HOLIDAY_TIME)
        settime.calc_time()
    ...
    return render_template('attendance/index.html', title='ホーム', cal=cal, shinseis=shinseis, y=y, m=m, form=form, form_month=form_month,
                           ...
                           workday_data=workday_data, cnt_attemdance=cnt_attemdance, reload_y=reload_y,
                           ...
                           dsp_page=dsp_page, STAFFID=STAFFID, template1=template1, template2=template2, AttendanceDada=AttendanceDada,
                           working_time=working_time, ln_s_kyori=ln_s_kyori, workday_count=workday_count, holiday_work=holiday_work,
                           intFlg=intFlg)
